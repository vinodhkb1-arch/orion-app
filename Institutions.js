...
export default function Institutions({ instData, setInstData, basket, addToBasket }) {
  ...
  const fetchData = (yf, yt, sq, sf) => {
    setLoading(true);
    const url = sq.trim()
      ? `/api/institutions/search?q=${encodeURIComponent(sq)}&field=${sf}&year_from=${yf}&year_to=${yt}`
      : `/api/institutions/top?year_from=${yf}&year_to=${yt}`;
  ...
  const reset = () => { setQ('');setField('name');setYF(2000);setYT(2025);setPL(50);setInstData({rows:[],yearFrom:2000,yearTo:2025});setSel(null);setTrends([]); };
