...
function buildInstExportQuery(institutionIds, yearFrom, yearTo) {
  const ids = institutionIds.join(', ');
  return `-- ORION export: works for institution basket
-- Institution IDs: ${ids}
-- Year range: ${yearFrom}–${yearTo}
--
-- This query returns the unique work_ids of all papers affiliated
-- with the selected institutions in the given year range.
-- Use work_id to join with other tables in cwts-leiden.openalex_2025aug.
-- See the Guide tab for examples.

SELECT DISTINCT wai.work_id
FROM \`cwts-leiden.openalex_2025aug.work_affiliation_institution\` wai
JOIN \`cwts-leiden.openalex_2025aug.work\` w ON wai.work_id = w.work_id
WHERE wai.institution_id IN (${ids})
...
export default function InstBasket({ basket, removeFromBasket, basketData, setBasketData }) {
  ...
  const analyze = () => {
    ...
    apiFetch('/api/basket/institutions/analyze', {
      method: 'POST',
    ...
    <div className="page">
      ...
      {basket.length === 0
        ...
        : <>
          ...
          {showQuery && (
            <QueryModal
              sql={buildInstExportQuery(basket.map(b => b.institution_id), yearFrom, yearTo)}
